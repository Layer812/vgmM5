...
extern "C" int puff(unsigned char *dest, unsigned long *destlen, const unsigned char *source, unsigned long *sourcelen);
...
void IRAM_ATTR wrap_psg_tick(ISoundChip* obj, int32_t* l, int32_t* r) { psg_engine_update(&g_psg_engine, l, r); }
...
class PsgChip final : public ISoundChip { public: PsgChip() : ISoundChip(wrap_psg_tick) {} void write(uint8_t port, uint16_t reg, uint8_t data) override { psg_engine_write_sn76489(&g_psg_engine, data); } };
...
class SccChip final : public ISoundChip { public: SccChip() : ISoundChip(wrap_scc_tick) {} void write(uint8_t port, uint16_t reg, uint8_t data) override { scc_engine_write(&g_scc_engine, reg, data); } };
...
void playSong(int index) {
    ...
    File f = FFat.open(songs[index], "r");
    if (!f) { isPlaying = false; return; }

    active_chip_count = 0;
    vgm_file_size = f.size();
    if (vgm_data != nullptr) { free(vgm_data); vgm_data = nullptr; }
    
    String lname = songs[index]; lname.toLowerCase();
    bool isVgz = lname.endsWith(".vgz");
    uint32_t uncompressed_size = vgm_file_size;

    if (isVgz) {
        if (f.size() < 4) { f.close(); isPlaying = false; return; }
    ...
    if (!vgm_data) { f.close(); isPlaying = false; return; }
    
    if (isVgz) {
        uint8_t* comp_data = (uint8_t*) heap_caps_malloc(vgm_file_size, MALLOC_CAP_SPIRAM);
        if (!comp_data) comp_data = (uint8_t*) malloc(vgm_file_size);
        if (!comp_data) { free(vgm_data); vgm_data = nullptr; f.close(); isPlaying = false; return; }
        f.read(comp_data, vgm_file_size); f.close();

        int gzipOffset = skipGzipHeader(comp_data, vgm_file_size);
        if (gzipOffset < 0) { free(comp_data); free(vgm_data); vgm_data = nullptr; isPlaying = false; return; }

        unsigned long destlen = uncompressed_size; unsigned long sourcelen = vgm_file_size - gzipOffset;
        int ret = puff(vgm_data, &destlen, comp_data + gzipOffset, &sourcelen);
        free(comp_data); 
        if (ret != 0) { free(vgm_data); vgm_data = nullptr; isPlaying = false; return; }
        vgm_file_size = destlen;
    } else {
        f.read(vgm_data, vgm_file_size); f.close();
    }

    uint8_t* header = vgm_data;
    ...
    fade_vol = 0.0f;
    fade_step = 1.0f / (actual_sample_rate * 0.02f); // 20ms micro fade-in
    


    // アンプの起動ノイズ（ポップ音・0.1秒のゆがみ）を回避するため、無音を約500ms流してI2Sを安定させる
    static const int16_t silence_buf[512 * 2] = {0};
    for (int i = 0; i < 43; i++) { // 43 * 11.6ms = 約500ms
        while (!M5.Speaker.playRaw(silence_buf, 512 * 2, actual_sample_rate, true, 1, 0, false)) {
            vTaskDelay(1);
        }
    }
...
void stopSong() { isPlaying = false; pcm_engine_free(&g_pcm_engine); }
...
void processVGM() {
    if (!isPlaying || vgm_data == nullptr) return;

    while (vgm_ptr < vgm_file_size && waitSamples == 0) {
        uint8_t cmd = vgm_data[vgm_ptr++]; 
        switch (cmd) {
            case 0x50: { uint8_t val = readByte(); psg_wrapper.write(0, 0, val); break; }
            case 0x51: 
            ...
            case 0xA0: { uint8_t reg = readByte(); uint8_t val = readByte(); ssg_wrapper.write(0, reg, val); break; }
            ...
            case 0xD2: { uint8_t port = readByte(); uint8_t reg = readByte(); uint8_t val = readByte(); scc_engine_write(&g_scc_engine, reg, val); break; }
            ...
            default:
                ...
                else if (cmd >= 0x30 && cmd <= 0x3F) vgm_ptr += 1;
...
void IRAM_ATTR processAudioBlock() {
    ...
    for (int i = 0; i < BUFF_SIZE; i++) {
        ...
        // 全チップの合計音量を適切なレベルにスケールダウン（桁あふれ防止）
        mix_l /= 6;
        mix_r /= 6;
...
void loop() {
    ...
    if (millis() - last_update > 50) {
        ...
        if (clickCount > 0 && (millis() - lastClickTime > 400)) {
            if (clickCount == 1) { isPlaying = !isPlaying; if (isPlaying) { prebuffering = true; playSong(currentSong); } }
            else if (clickCount >= 2) { isPlaying = false; vTaskDelay(20); currentSong = (currentSong + 1) % numSongs; prebuffering = true; playSong(currentSong); }
            clickCount = 0;
        }
    }

    static unsigned long last_heartbeat = 0;
    if (!isPlaying) { vTaskDelay(10); return; }
    

    if (prebuffering) {
        if (wav_count < (WAV_BUFF_COUNT - 4)) { vTaskDelay(1); return; }
        else {
            prebuffering = false;
        }
    }
    
    // 再生キューイング処理は専用タスクに移動しました
    vTaskDelay(10); 
}
...
static int32_t onWrite(uint32_t lba, uint32_t offset, uint8_t* buffer, uint32_t bufsize) {
    ...
    // USB MSC typically writes whole sectors at once. 
    // If offset == 0 and we are writing a whole sector, we should erase it first.
    // In TinyUSB, onWrite is usually called block by block (e.g. 4096 bytes).
    if (offset == 0) {
        wl_erase_range(s_wl_handle, address, bufsize);
    }
...
void enter_usb_mode() {
    ...
    // 一度FFatをマウントしてフォーマット済みであることを担保してからアンマウント
    FFat.begin(true);
    FFat.end();
...
void setup() {
    ...
    if (M5.BtnA.isPressed()) {
        enter_usb_mode(); // ここで無限ループに入り、以下のオーディオ処理は実行されない
    }
    ...
    spk_cfg.task_priority = configMAX_PRIORITIES - 1; // ★追加: 内部I2Sタスクの優先度を最大化
