...
static void mdx_driver_note_on(struct mdx_driver *r, int track_num) {
	...
	if(track_num < 8) {
		...
		if(track->note >= 0 && track->note < 96 && r->pdx_file) {
			// MDX仕様: PCMトラックの @指定(voice_num) はPDXスロットのオフセットとして加算する
			int pdx_slot = track->note;
			if (track->voice_num > 0) {
				pdx_slot += track->voice_num;
			}
